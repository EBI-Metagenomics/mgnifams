...
if __name__ == "__main__":
    ...
    parser.add_argument("query_results_dir", help="Path to the output query results dir generated by the query_mgnprotein_db module")
    ...
    if not os.path.exists(args.query_results_dir):
        raise FileNotFoundError(f"The folder {args.query_results_dir} does not exist.")

    if not os.path.exists(args.pfam_mapping):
        raise FileNotFoundError(f"The file {args.pfam_mapping} does not exist.")

    pfam_mapping_df = pd.read_csv(args.pfam_mapping, sep='\t', header=None, names=['id', 'name'])
    ...
    query_results_files = os.listdir(args.query_results_dir)
    ...
    for tsv in query_results_files:
        file_path = os.path.join(args.query_results_dir, tsv)
