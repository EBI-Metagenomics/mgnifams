...
def create_csv(input_tsv, output_csv):
    df = pd.read_csv(input_tsv, sep='\s+', comment='#', header=None)
    
    # Split first column using "_" as separator and keep the first part
    df[0] = df[0].apply(lambda x: x.split('_')[0])
    
    # Split third column using "|" as separator and keep the second part
    df[3] = df[3].apply(lambda x: x.split('|')[2])
    ...
    df_out = df[[0,3,'None', 'Uniprot SP', 'False']].copy()
