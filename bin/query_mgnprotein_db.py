...
def fetch_protein_metadata(db_params, all_sequences):
    ...
    try:
        ...
        for i in range(0, len(sequence_list), BATCH_SIZE):
            ...
            cursor.execute(
                "SELECT mgyp, metadata FROM sequence_explorer_protein WHERE mgyp = ANY(%s::bigint[])", (batch,)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--threads", type=int, default=1, help="Number of parallel threads for writing family TSV files"
    )
